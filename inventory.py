...
st.title("📊 AS TAT 통합 관리 시스템 (UI 강제 복구)")
...
# --- [TAB 0] 마스터 관리 (업로드 박스를 최상단으로 뺐습니다) ---
with tab0:
    st.subheader("📋 마스터 데이터 등록")
    
    # 이 위젯이 "Drag and drop file here" 박스입니다. 
    # 탭 클릭 시 가장 먼저 보여야 합니다.
    m_file = st.file_uploader(
        "공급업체/분류 정보가 담긴 마스터 파일을 업로드하세요.", 
        type=['xlsx', 'csv'], 
        key="master_uploader_v9"
    )
...
# --- [TAB 1] 입고 처리 (실시간 진행사항 포함) ---
with tab1:
    st.subheader("📥 AS 입고")
    i_file = st.file_uploader("AS 입고 CSV 업로드", type=['csv'], key="in_up_v9")
    
    if i_file and st.button("🚀 입고 시작", use_container_width=True):
        if "master_lookup" not in st.session_state:
            st.error("⚠️ 마스터 관리를 먼저 진행해주세요.")
        else:
            ...
            try:
                ...
                while True:
                    ...
                    offset += len(res.data)
                    ui_msg.info(f"🔍 중복 데이터 대조 중... ({offset:,} 건)")
                ...
                for i, (_, row) in enumerate(as_in.iterrows()):
                    ...
                    if len(recs) >= 200:
                        ...
                        recs = []
                        ui_msg.warning(f"🚀 데이터 저장 중... ({i+1:,} / {total_in:,})")
                        ui_prog.progress(min((i+1)/total_in, 1.0))
                
                if recs: supabase.table("as_history").insert(recs).execute()
                ui_msg.success(f"✅ 입고 완료 (중복제외 신규: {total_in-dup_cnt:,}건)")
                ui_prog.progress(1.0)
            except Exception as e: st.error(f"오류: {e}")

# --- [TAB 2] 출고 처리 (실시간 진행사항 포함) ---
with tab2:
    st.subheader("📤 AS 출고")
    out_file = st.file_uploader("출고 엑셀 업로드", type=['xlsx'], key="out_up_v9")
    
    if out_file and st.button("🚀 출고 데이터 반영", use_container_width=True):
        ui_msg, ui_prog = st.empty(), st.progress(0)
        try:
            ...
            if update_list:
                for idx, item in enumerate(update_list):
                    ...
                    if idx % 50 == 0:
                        ui_msg.warning(f"🔄 DB 반영 중... ({idx:,} / {len(update_list):,})")
                        ui_prog.progress(min(idx/len(update_list), 1.0))
                ui_msg.success(f"✅ {len(update_list):,}건 반영 완료")
                ui_prog.progress(1.0)
        except Exception as e: st.error(f"오류: {e}")

# --- [TAB 3] 리포트 생성 (실시간 진행사항 포함) ---
with tab3:
    st.subheader("📊 리포트 생성")
    ...
    if st.session_state.get("data_ready"):
        ...
        with c1: st.download_button("📥 출고완료", st.session_state.bin_tat, "done.xlsx", use_container_width=True)
        with c2: st.download_button("📥 미출고", st.session_state.bin_stay, "pending.xlsx", use_container_width=True)
        with c3: st.download_button("📥 전체", st.session_state.bin_total, "all.xlsx", use_container_width=True)
