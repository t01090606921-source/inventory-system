...
with tab0:
    ...
    # [핵심] 여기에 파일 업로드 박스가 나타납니다. 
    m_file = st.file_uploader(
        "마스터 엑셀(XLSX) 또는 CSV 파일을 선택하세요", 
        type=['xlsx', 'csv'], 
        key="master_upload_final_v11"
    )
...
with tab1:
    ...
    i_file = st.file_uploader("AS 입고 CSV 파일 업로드", type=['csv'], key="in_up_final_v11")
...
with tab2:
    ...
    out_file = st.file_uploader("출고 결과 엑셀 업로드", type=['xlsx'], key="out_up_final_v11")
