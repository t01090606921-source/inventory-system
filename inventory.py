...
def check_password():
    ...
    st.title("🏭 디지타스 창고 재고관리 (Ver.11.4)")
...
# --- [강제 초기화 버튼 - 진단용] ---
# 에러가 나서 탭을 못 누를 때를 대비해 사이드바나 최상단에 배치
with st.sidebar:
    if st.button("🔄 캐시 데이터 강제 삭제 (에러 시 클릭)", type="primary"):
        st.cache_data.clear()
        st.rerun()

# --- [핵심 수정] 대용량 데이터 가져오기 (정렬 조건 제거) ---
def fetch_all_data(table_name):
    ...
    while True:
        try:
            # [수정] .order("id") 제거 -> id 컬럼 없는 테이블(매핑정보 등) 에러 방지
            ...
            # 에러 발생 시 콘솔에만 찍고 멈추지 않음 (부분 데이터라도 살림)
...
def calculate_stock_snapshot(df_log, df_mapping, df_master, df_details):
    ...
    # [수정] 매핑정보가 비었거나 컬럼이 없을 경우 대비 (KeyError 방지)
    if df_mapping.empty:
