...
def check_password():
    ...
    st.title("🏭 디지타스 창고 재고관리 (Ver.10.0)")
...
def main():
    ...
    with tab1:
        ...
        if not df_log.empty:
            ...
            st.download_button(
                label="📥 전체 입출고 이력 다운로드",
                data=csv_data,
                file_name=f"전체이력_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
